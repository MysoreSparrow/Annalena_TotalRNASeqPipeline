...
do
	...
	cat *22030a0**${i}_01*_L00[1,2]_R1_001.fastq.gz > 22030a0${i}_01_S1_lm_R1.fastq.gz
	# merge for L1R2 and L2R2 and obtain lanemerged (lm) R2 file
	cat *22030a0**${i}_01*_L00[1,2]_R2_001.fastq.gz > 22030a0${i}_01_S1_lm_R2.fastq.gz
	
	# For 02
	# merge for L1R1 and L2R1 and obtain lanemerged (lm) R1 file
	cat *22030a0**${i}_02*_L00[1,2]_R1_001.fastq.gz > 22030a0${i}_02_S1_lm_R1.fastq.gz
	# merge for L1R2 and L2R2 and obtain lanemerged (lm) R2 file
	cat *22030a0**${i}_02*_L00[1,2]_R2_001.fastq.gz > 22030a0${i}_02_S1_lm_R2.fastq.gz

	# Now perform Sample merging of the lane merged files

	# merge 01_lm_R1 and 02_lm_R1 into single samplemerged(sm) R1 file
	cat *22030a0**${i}*_0[1,2]_S1_lm_R1.fastq.gz > AJ0${i}_sm_R1.fastq.gz
	# merge 01_lm_R2 and 02_lm_R2 into single samplemerged(sm) R2 file
	cat *22030a0**${i}*_0[1,2]_S1_lm_R2.fastq.gz > AJ0${i}_sm_R2.fastq.gz
done
