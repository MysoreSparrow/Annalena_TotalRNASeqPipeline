...
d5_UninfectedVSInfected_coldata <-
  coldata[is.element(coldata$Sample_Name, c("AJ_01", "AJ_02",
                                            "AJ_03", "AJ_04",
                                            "AJ_11", "AJ_12",
                                            "AJ_15", "AJ_16")), ]
# # Select Rows by list of column values
d5_UninfectedVSInfected_countsmatrix <- subset(countsmatrix,
                                               select = c("AJ_01", "AJ_02",
                                                          "AJ_03", "AJ_04",
                                                          "AJ_11", "AJ_12",
                                                          "AJ_15", "AJ_16"))
...
```{r fig.height=6, fig.width=6 PCA Plot Aesthetics}
# PCA Plot
...
```{r PC Plot: PC1 vs PC2}
(PCAplot_pc12 <- ggplot(
  pcaData,
  aes(x = PC1,y = PC2, color = Sample_Name, label = Sample_Name, shape = Condition)) +
...
savingFunction(PCAplot_pc12, metadatacolumn = "Condition")

(PCAplot_pc12 <- ggplot(
  pcaData,
  aes(x = PC1,y = PC2, color = Sample_Name, label = "", shape = Condition)) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  ggtitle(glue("PCA: {Comparison}")) +
  scale_colour_manual(values = color_values) +
...
```{r PC Plot: PC3 vs PC4}
(PCAplot_pc34 <- ggplot(
  pcaData,
  aes(x = PC3,y = PC4, color = Sample_Name, label = Sample_Name, shape = Condition)) +
...
savingFunction(PCAplot_pc34, metadatacolumn = "Condition")

(PCAplot_pc34 <- ggplot(
  pcaData,
  aes(x = PC3,y = PC4, color = Sample_Name, label = "", shape = Condition)) +
  xlab(paste0("PC3: ", percentVar[3], "% variance")) +
  ylab(paste0("PC4: ", percentVar[4], "% variance")) +
  ggtitle(glue("PCA: {Comparison}")) +
  scale_colour_manual(values = color_values) +
...
ggsave(
  filename = file.path(Comparison_Path, "/Volcano1_L2fcVsPvalue.png"),
  plot = volcano1,
  dpi = 300,
  width = 5,
  height = 5,
...
ggsave(
  filename = file.path(Comparison_Path, "/Volcano2_L2fcVsPadj.png"),
  plot = volcano2,
  dpi = 300,
  width = 10,
  height = 10,
...
# mat <- counts(dds, normalized = TRUE)[(sigsdf$symbol) %in% rownames(counts(dds)), ]
...
```{r}
sigs3df <- resdf[(resdf$baseMean > 5000) & (abs(resdf$log2FoldChange) > 2) & (resdf$pvalue < 0.05), ]
...
GO_UPReg_Heatplot <- plot(heatplot(GO_UPRegResults, foldChange = 1,label_format = 20,
                                   showCategory = 25))
...
```{r fig.height=12, fig.width=12}
emapplot_cluster(edox2, showCategory = 50, repel = TRUE, layout = 'star',
                 color = "p.adjust")
```
